  ...
  const ActualizarContrato = async () => {
    ...
    try {
      ...
      // CASO 3: Error de red o CORS (response es undefined)
        ...
        // Si la petición se hizo pero no hubo respuesta legible
        // A veces esto es CORS. Si estás SEGURO de que se actualiza, podrías descomentar la siguiente línea:
        // navigate("/zona-privada"); 
        setError("El servidor no respondió correctamente. Verifica si los datos se guardaron.");
      } 
              ...
              <div className="p-6 space-y-4">
                    ...
                    {seleccion.movil.map((l, i) => (
                      <div key={l.uid} className="flex flex-col text-sm border-b border-slate-50 pb-2 mb-2 last:border-0">
                        ...
                        <div className="text-xs text-slate-400 mt-1 flex justify-between">
                          {l.tarifa ? (l.tarifa.gb_datos <= 0 ? '∞' : `${l.tarifa.gb_datos}GB`) : 'Sin tarifa'}
                          {l.tarifa?._legacy && (
                            <span className="text-red-500 ml-1">(No disp.)</span>
                          )}
                ...
                <div className="pt-6 mt-2 border-t border-slate-100">
                  ...
                  <button
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:cursor-not-allowed text-white py-4 rounded-xl font-bold text-lg"
                  >
