...
import { getContratoUsuario } from "../api/contratosApi";
...
function ContratoComponent() {
    ...
    if (loading) return <p className="text-center mt-10 text-gray-500">Cargando contrato...</p>;
    if (error) return <p className="text-center mt-10 text-red-600">{error}</p>;

    if (!contratoData) {
        return (
            <div className="text-center mt-10">
                <p className="mb-4 text-gray-700 text-lg">No tienes ningún contrato activo.</p>
                <button
            ...
            <div className="w-full max-w-7xl bg-white shadow-xl rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <div className="px-6 py-6 grid md:grid-cols-3 gap-6 border-b border-gray-100 bg-white">
                    <div>
                        ...
                        <p className="font-semibold text-gray-800">{contrato.calle_y_n}</p>
                    </div>

                    <div>
                        <p className="text-xs text-gray-500">Ciudad</p>
                        <p className="font-semibold text-gray-800">{contrato.ciudad}</p>
                    </div>

                    <div>
                        <p className="text-xs text-gray-500">Código Postal</p>
                        <p className="font-semibold text-gray-800">{contrato.codigo_postal}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 py-6 bg-white">
                    ...
                    <div className="rounded-xl border border-yellow-400 shadow-sm hover:shadow-lg transition bg-white">
                        ...
                        <div className="p-5 text-center">
                            <p className="text-3xl font-extrabold text-yellow-400 ">{servicios.tv.nombre_paquete}</p>
                            <p className="mt-1">
                ...
                <div className="px-6 py-4 bg-white flex justify-end">
                    ...
                    >
                        <Pencil size={16} /> {/* Ícono de Lápiz */}
                        Modificar
                    </Link>

                    <button
                        // Aquí iría la función para cancelar el contrato
                    ...
                    >
                        <X size={16} /> {/* Puedes añadir un ícono como una 'X' para Cancelar */}
                        Cancelar
                    </button>
                </div>
